...
namespace MSL.pages.frpProviders
{
    ...
    public partial class Custom : Page
    {
            ...
            try
            {
                ...
                if (EasyMode.IsChecked == true)
                {
                    ...
                    else
                    {
                        ...
                        LogHelper.WriteLog($"隧道 {number} 的frpc.toml内容已生成，准备写入文件。");
                        Directory.CreateDirectory("MSL\\frp\\" + number);
                        File.WriteAllText($"MSL\\frp\\{number}\\frpc.toml", FrpcConfig);
                        LogHelper.WriteLog($"配置文件 frpc.toml 已成功写入到 MSL\\frp\\{number} 目录。");
                        SetFrpcPath(number);
                    }
                }
                else
                {
                    LogHelper.WriteLog("当前为直接配置文件(CustomMode)模式。");
                    //直接丢配置文件模式
                    Directory.CreateDirectory("MSL\\frp\\" + number);
                    File.WriteAllText($"MSL\\frp\\{number}\\frpc.toml", ConfigBox.Text);
                    LogHelper.WriteLog($"用户提供的直接配置文件 frpc.toml 已成功写入到 MSL\\frp\\{number} 目录。");
                    SetFrpcPath(number);
                }
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog($"在 OKBtn_Click 事件处理中发生严重错误: {ex.ToString()}", LogLevel.FATAL);
        ...
        {
            LogHelper.WriteLog($"开始为隧道 {number} 设置最终配置路径和名称。");
            string sn = await MagicShow.ShowInput(Window.GetWindow(this), "给此隧道取一个名字吧：", "我的自定义Frp节点");
            if (sn == null)
            {
                LogHelper.WriteLog($"用户取消了为隧道 {number} 命名，配置流程中止。", LogLevel.WARN);
                return;
            }
            ...
            try
            {
                ...
                {
                    LogHelper.WriteLog("未检测到 MSL\\frp\\config.json 文件，正在创建新文件。", LogLevel.WARN);
                ...
                {
                    LogHelper.WriteLog("用户选择使用自定义Frpc客户端。");
                    //选择文件对话框
                    ...
                    if (openFileDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        LogHelper.WriteLog($"用户选择了Frpc客户端: {filePath}。");
                        File.Copy(filePath, @"MSL/frp/frpc_custom.exe", true);
                        LogHelper.WriteLog("自定义Frpc客户端已成功复制到 MSL/frp/frpc_custom.exe。");
                        ...
                        LogHelper.WriteLog($"已为隧道 {number} 添加配置条目(自定义客户端模式)。");
                    }
                    else
                    {
                        LogHelper.WriteLog("用户取消选择自定义Frpc客户端，配置流程中止。", LogLevel.WARN);
                        return;
                    }
                }
                else
                {
                    LogHelper.WriteLog("用户选择使用官方Frpc客户端。");
                    JObject keyValues = new JObject()
                    {
                        ["frpcServer"] = -1,
                        ["name"] = "自定义隧道(官方客户端) - " + sn
                    ...
                    jobject.Add(number.ToString(), keyValues);
                    LogHelper.WriteLog($"已为隧道 {number} 添加配置条目(官方客户端模式)。");
                }

                //最后结束
                string convertString = Convert.ToString(jobject);
                File.WriteAllText(@"MSL\frp\config.json", convertString, Encoding.UTF8);
                LogHelper.WriteLog("最终配置 config.json 已成功保存。");
                await MagicShow.ShowMsgDialogAsync(Window.GetWindow(this), "隧道配置成功，请您点击“启动内网映射”以启动映射！", "信息");
                LogHelper.WriteLog($"隧道 {number} (名称: {sn}) 的配置流程已全部成功完成。");
                Window.GetWindow(this).Close();
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog($"在 SetFrpcPath 方法中发生严重错误: {ex.ToString()}", LogLevel.FATAL);
