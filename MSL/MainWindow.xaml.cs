...
namespace MSL
{
    ...
    public partial class MainWindow
    {
        ...
        private void AsyncLoadEvent()
        {
            ...
            try
            {
                serverLink = Functions.Get("", out string _, "https://msl-server.oss-cn-hangzhou.aliyuncs.com/",true);
                try
                {
                    if (((int)((JObject)JsonConvert.DeserializeObject(Functions.Get("", out string _)))["status"]) != 200)
                    {
            ...
            try
            {
                ...
                string aaa = Functions.Get("query/update", out string _);
                if (aaa.Contains("v"))
                {
                    aaa = aaa.Replace("v", "");
                }
                ...
                if (newVersion > version)
                {
                    ...
                    var updatelog = Functions.Get("query/update/log", out string _);
                    Dispatcher.Invoke(() =>
                    {
        ...
        private void UpdateApp(string aaa)
        {
            ...
            string aaa1 = Functions.Get("download/update", out string _);
