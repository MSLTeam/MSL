...
namespace MSL.utils
{
    ...
    public static class LogHelper
    {
        ...
        public class Write
        {
            ...
            public static void WriteLog(string content, string prefix)
            {
                ...
                lock (_lock)
                {
                    try
                    {
                        ...
                        // 构造日志条目，格式：[时间戳] [级别] 内容
                        string logEntry = $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss.ff}] [{prefix}] {content}";

                        // 使用 'using' 语句和 StreamWriter 将日志条目追加到当前日志文件中
                        using StreamWriter writer = new(_currentLogFilePath, true, Encoding.UTF8);
